...
class EB_RELION(CMakeMake):
    ...
    def configure_step(self):
        ...
        # Create torch directory in TEMPORARY location (like conda)
        self.temp_torch_path = os.path.join(tempfile.gettempdir(), f'relion_torch_{os.getpid()}')
        os.makedirs(self.temp_torch_path, exist_ok=True)
        self.log.info(f"Created temporary torch directory: {self.temp_torch_path}")
        ...
        # Set up cmake options using TEMPORARY paths
        ...
        # Use TEMPORARY torch path for cmake
        self.cfg.update('configopts', f' -DTORCH_HOME_PATH={self.temp_torch_path}')
        self.log.info(f"Set temporary torch path for cmake: {self.temp_torch_path}")
    ...
    def install_step(self):
        """Install RELION and then move conda environment and torch to final locations."""
        self.log.info("=== RELION INSTALL STEP START ===")

        super().install_step()

        # Move conda environment from temporary to final location
        if hasattr(self, 'temp_conda_path') and os.path.exists(self.temp_conda_path):
        ...
        # Move torch directory from temporary to final location
        if hasattr(self, 'temp_torch_path') and os.path.exists(self.temp_torch_path):
            final_torch_path = os.path.join(self.installdir, 'torch')
            self.log.info(f"Moving torch from {self.temp_torch_path} to {final_torch_path}")

            os.makedirs(self.installdir, exist_ok=True)
            shutil.move(self.temp_torch_path, final_torch_path)

            self.torch_path = final_torch_path
            self.log.info(f"Torch directory successfully moved to: {final_torch_path}")
        else:
            self.log.info("No temporary torch directory to move")
    ...
    def make_module_extra(self):
        """Add conda environment and torch to module."""
        ...
        # Add torch home using FINAL path
        if self.torch_path and os.path.exists(self.torch_path):
            txt += self.module_generator.set_environment('TORCH_HOME', self.torch_path)
        else:
            # Fallback to expected location if torch_path not set
